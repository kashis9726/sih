...
const Header: React.FC<HeaderProps> = ({ onChatToggle }) => {
  ...
  // Local notifications from modules (mentorship, etc.)
  const userNotifs = useMemo(() => {
    try {
      const raw = JSON.parse(localStorage.getItem('notifications') || '[]');
      const arr = Array.isArray(raw) ? raw : [];
      return arr.filter((n: any) => n.userId === user?.id);
    } catch {
      return [] as any[];
    }
  }, [user?.id, showNotifs]);
  ...
  const unreadUserNotifs = userNotifs.filter((n: any) => !n.readAt);
  const notifCount = jobNotifs.length + blogNotifs.length + upcomingEvents.length + unreadUserNotifs.length;
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative">
            <button onClick={() => {
                ...
                if (!showNotifs) {
                  // mark user notifications as read when opening
                  try {
                    const raw = JSON.parse(localStorage.getItem('notifications') || '[]');
                    const arr = Array.isArray(raw) ? raw : [];
                    const updated = arr.map((n: any) => n.userId === user?.id && !n.readAt ? { ...n, readAt: new Date().toISOString() } : n);
            ...
            {showNotifs && (
              <div className="absolute right-0 mt-2 w-[360px] bg-white rounded-xl shadow-elev-2 border border-gray-200 p-2 z-20">
                ...
                <div className="max-h-96 overflow-auto space-y-2">
                  {/* Module notifications (e.g., mentorship) */}
                  {userNotifs.map((n: any) => (
                    <div key={n.id} className="flex items-start gap-3 px-2 py-2 rounded-lg hover:bg-gray-50">
                      <div className="p-2 rounded-lg bg-indigo-50 text-indigo-700"><Bell className="h-4 w-4" /></div>
                      <div className="min-w-0">
                        <div className="text-sm font-semibold text-gray-900 truncate">{n.type?.toString().replace(/_/g,' ')}</div>
